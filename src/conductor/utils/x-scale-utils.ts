...
import { tuningPitch100 } from "../utils.vers";
import { globalVars, SysSettings } from "../x-var";
import * as XArrayUtils from "./x-array-utils";
 ...
 */
export function bitScaleToSymScale(key: IKey, bin: bin12): (IKey | undefined)[] {
  // keyを開始としたsym列
  const iKeys = XArrayUtils.shiftArray(globalVars.iKey, key);
  return bin.map((b, i) => b === 1 ? iKeys[i] : undefined);
}
 ...
 */
export function createExtensionBin(key: IKey, bin: bin12, addKeys: IKey[]): (IKey | undefined)[] {
  // keyを開始としたsym列
  const iKeys = XArrayUtils.shiftArray(globalVars.iKey, key);
  return bin.map((b, i) => b === 1 || addKeys.includes(iKeys[i]) ? iKeys[i] : undefined);
}
 ...
 * // }
 */
export function getMidiNoteFromFingering(fingering: NumberOrUfd[], tuningPitches: number[]): {
  ...
  // 元ネタ音解析
  const originKeyArr: IKey[] = []; // 元ネタのキーsym（複数音）
  const originNoteNumArr: number[] = []; // 元ネタのnoteナンバー（0開始）
  const originFretArr: number[] = []; // 元ネタのフレット配列（複数音）
  const originStringArr: number[] = []; // 元ネタの弦index配列（複数音）
  fingering.forEach((fret, i) => {
    if (fret !== undefined && fret >= 0) {
      const noteNumber = tuningPitches[i] + fret;
      originKeyArr.push(tuningPitch100[noteNumber].keySym);
      originNoteNumArr.push(noteNumber - tuningPitches[tuningPitches.length - 1]); // 0開始にするため引く
      originFretArr.push(fret);
      originStringArr.push(i);
    }
  });
  return {
    originKeyArr, // IKey[]
 ...
 * // }
 */
export function createActiveNoteOnlyList(boardFullArr: (IKey | undefined)[]) {
  // 有効音のみのindexを値に持った配列
  return {
    activeInIndexes: boardFullArr.map((m, i) => m !== undefined ? i : undefined).filter(f => f !== undefined),
    activeInKeys: boardFullArr.map((m) => m !== undefined ? m : undefined).filter(f => f !== undefined)
  }
}
 ...
 */
export function addExtendKeyIntoScale(
  boardFullArr: (IKey | undefined)[],
  iKeysWithTuningStart: IKey[],
  originKeyArr: IKey[],
) {
  const extendScaleFullArr = structuredClone(boardFullArr);
  originKeyArr.forEach(k => {
    if (!boardFullArr.includes(k)) {
      const startIndex = iKeysWithTuningStart.indexOf(k);
      for (let i = startIndex; i < extendScaleFullArr.length; i = i + 12) {
        extendScaleFullArr[i] = k;
      }
    }
  });
  return extendScaleFullArr;
}
 ...
 * // }
 */
export function createBoardFullLine(tuningPitches: number[], scale: StyleScaleX, tuningMinKey: IKey): {
  ...
  // スケール本体
  const scaleBaseBin = scale.bin;// ScaleList[scale.scale].bin;

  // スケールのkeyを開始としたsym列
  const iKeysWithKeyStart = XArrayUtils.shiftArray(globalVars.iKey, scale.key);

  // チューニング最低音を開始とした12sym列
  const iKeysWithTuningStart = XArrayUtils.shiftArray(globalVars.iKey, tuningMinKey);

  // bin12を作成（指定キーから開始）
  const extendSymBin = scaleBaseBin.map((b, i) => b === 1 ? iKeysWithKeyStart[i] : undefined);

  // チューニング最低最大音ノートナンバー
  const tuningMaxNote = tuningPitches[0] + SysSettings.maxTopFret + 1;
  const tuningMinNote = tuningPitches[tuningPitches.length - 1];

  // 最低解放弦のキーとスケールのキーの差分をとり、最低弦の開放から始める配列にシフト
  const lowerStartIKeys = XArrayUtils.shiftArray(globalVars.iKey, tuningPitch100[tuningMinNote].keySym);
  const diffKeyIndex = 12 - lowerStartIKeys.indexOf(scale.key);
  const lowestStartScale = XArrayUtils.shiftArrayAsIndex(extendSymBin, diffKeyIndex);

  // スケールを一列boardに当てはめる（最低限の解放から始まる配列）
  const repeatedArray = Array.from({ length: Math.floor((tuningMaxNote - tuningMinNote) / 12) }, () => lowestStartScale).flat();
  const boardFullArr = repeatedArray.concat(lowestStartScale.slice(0, (tuningMaxNote - tuningMinNote) % 12));
  // console.log('boardFullArr', boardFullArr)
