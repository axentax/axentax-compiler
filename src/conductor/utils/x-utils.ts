 ...
 */
export function decimalizeAdd(a: number, b: number): number {
 ...
 */
export function decimalizeMul(a: number, b: number): number {
 ...
 */
export function decimalizeDiv(a: number, b: number): number {
 ...
 */
export function decimalizeSub(a: number, b: number): number {
 ...
 */
export function innerTrimerForStyleKey(token: string): string {
  const trimmed = token
    .replace(/\s+\)/g, ')') // 閉じ括弧前の空白を除去
    .replace(/\(\s+/g, '(') // 開き括弧後の空白を除去
    .replace(/\s*,\s*/g, ',') // カンマの前後の空白を除去
 ...
 */
export function resolveNonRegularKey2str(token: string): IKey {
  ...
  switch (token) {
    ...
    default: {
      /* istanbul ignore next: すべての非標準キーは上記のケースで処理されるため、default ケースは実際には到達しない */
      return token as IKey;
    }
  }
 ...
 */
export function resolveNonRegularKey3str(token: string): IKey {

  if (token.length === 3) {

    const _bs = token.replace(/^([CDEFGAB])(b#|#b)$/, '$1');
    if (_bs !== token) {
      return _bs as IKey;

    } else {
      const _ss = token.match(/^([CDEFGAB]#)#$/);
      if (_ss) {
        return searchNextKey(_ss[1] as IKey);

      } else {
        const _bb = token.match(/^([CDEFGAB])bb$/);
        if (_bb) {
          /* istanbul ignore next: ダブルフラット（bb）は通常の音楽理論では稀であり、実運用では使用されない */
          return searchPrevKey(_bb[1] as IKey, 2)
        }
      }
    }
  }
  ...
  // 3文字でない場合は2文字版の処理を使用
  return resolveNonRegularKey2str(token);
}
 ...
 * searchNextKey('C#') // 'D'
 */
export function searchNextKey(sym: IKey) {
  const keys = globalVars.iKey;
  const nextIndex = keys.indexOf(sym) + 1;
  return keys[nextIndex % 12]
}
 ...
 * searchPrevKey('C#') // 'C'
 */
export function searchPrevKey(sym: IKey, shift = 1) {
  const keys = globalVars.iKey;
  const prevIndex = keys.indexOf(sym) - shift;
  return keys[(12 + prevIndex) % 12]
}
