...
export interface TabObj {
  ...
  refMovedSlideTarget: (TabObj | undefined)[]; // --- slide総入れ替えのため可能予定 ---
...
export interface ExtensionViewProp {
  // step?: { tabAll: NumberOrUfd[], parsedStepList: ParsedStep[] },
  /** ステップ情報ID：ステップ処理の識別情報 */
  stepInfoId?: { id: number, orderCount: number },
  /** バレット情報：バレットの行とインデックス */
  bullet?: { row: string, index: number }; // , linePos: number, endPos: number },
  // map?: { shift: number, mapOpts: MapOpt[] }
}
...
export interface Bar {
  ...
  /** 全体tickの開始位置 ※layerには引き継がない? */
  // レガシー版では設定&算出しているが使用箇所が見当たらない
