...
// 以下三行を解放すると、..|2|2|2|2>>|5|5|5|5:to(continue) あたりでスライド(to)がおかしくなる
// @@ {C} >> { D~ }
// @@ {} >> { |2:1/16:map(0..3) @offset D } >> { E }
// @@ {C}

@@ { |||5:map(0..1):1/16 @offset }
// @@ {} // これを解放すると ..|2|2|2|2>>|5|5|5|5:to(continue) あたりでバグる(region bpm効かないとか、スライドの感じおかしくなるとか)
@@ {C}
@@ {C}
@@ {C}
@@ {C}
...
// @@ { C:stroke(12/12) |||||5:map(0 step 2 * 200):1/16 }:map(0..5)
// @@ { |||||5:map(0 step 2 * 20):1/16 }
// @@ { D } }{}
// @@ { C:step() }
// @@ 60 { 3|3|3|3|3|3>>7|7|7|7|7|7:1/8:stroke(2/1) E }
// @@ { {C D E}:bpm(200) {C D E}:bpm(..-190) }
// @@ 360 { r Em:1/4:stroke(1/2):to Em:stroke(2/3) }
// @@ { C /|||||r {}:bpm(150) { D D D }:bpm(+50..-90) }
...
// @@ 160 1/4 { |:0/1 } // 下のregionが region prop を反映せずに BPMが遅くなってしまう 一時対応
