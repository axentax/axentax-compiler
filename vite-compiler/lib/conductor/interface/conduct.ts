...
export interface Marks {
  ...
  usedBendRange: { start: number, end: number }[];
}
...
export interface Mixes {
  dualId: number, // dualチャンネルID（0, 1, 2）
  ...
  bendBank: {
    bendNormalList: BendMidiSetter[]; // 通常ベンド設定
    bendChannelList: BendMidiSetter[]; // チャンネル用ベンド設定
  }
  
  // /** trackAビブラート用 */
  // bendObjListForTrackA: BendViewObj[];
  // /** trackBチョーキング&ビブラート用 */
  // bendObjListForTrackB: BendViewObj[];
...
export interface Conduct {
  ...
  // precision: boolean; // ソロ構築時に無駄な処理（辞書構築など）をさせないためのフラグ
  /** ユーザーまたは作曲者入力の記譜法：元の記譜法テキスト */
  syntax: string;
  /** 設定情報：BPM、調性、チューニングなどの設定 */
  settings: Settings;
  /** ブロックリストの長さ：リージョンの総数 */
  regionLength: number; // dualにしてもlengthは変わらないため同じく使用できる
  ...
  /** 拡張情報：将来的な機能拡張用の情報 */
  extensionInfo: {
    stepInfoList: StepInfo[] // ステップ情報リスト
  };
...
export interface LocationInfo {
  line: number, // 開始行番号
  linePos: number, // 開始行内位置
  endLine: number, // 終了行番号
  endPos: number, // 終了行内位置
  type: CSymbolType, // シンボルタイプ（note、degree、bullet、blockStyle、flashなど）
  regionId: number, // リージョンID
  dualId: number, // dualチャンネルID
  /**
...
export interface BraceLocationInfo {
  ...
  styles: string[] // 適用されるスタイルリスト
  linesOfStyle: number[], // スタイルの行番号リスト
  linePosOfStyle: number[] // スタイルの行内位置リスト
}

/*
  カーソルが 100:20 の場合で以下のデータの場合 99 から見るべき
  末端から検索すればいいか
