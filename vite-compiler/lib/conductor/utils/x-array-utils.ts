 ...
 * @param val 追加する値
 */
export function orPush<T, V>(object: T, arrName: keyof T, val: V) {
  if (object[arrName] === undefined) {
    /* istanbul ignore next: 配列の初期化処理は通常の運用では object のプロパティが適切に初期化されているため、このパスは稀にしか実行されない */
    (object[arrName] as V[]) = [val]
  } else {
    (object[arrName] as V[]).push(val)
  }
}
 ...
 */
export function createNumberBetweenArray(start: number, end: number) {
 ...
 */
export function getReboundIndexValue<T>(numberArray: T[], index: number): T {
  // インデックスを配列の長さで正規化
  index = Math.abs(index) % (2 * (numberArray.length - 1));
  // 配列の長さを超える場合は折り返す
  if (index >= numberArray.length) {
    index = 2 * (numberArray.length - 1) - index;
  }
 ...
 */
export function getReboundResolvedIndex<T>(numberArray: T[], index: number): number {
  // インデックスを配列の長さで正規化
  index = Math.abs(index) % (2 * (numberArray.length - 1));
  // 配列の長さを超える場合は折り返す
  if (index >= numberArray.length) {
 ...
 */
export function findNthOneIndex<T>(arr: T[], n: number, v: T) {
  const oneIndices = arr.map((val, idx) => val === v ? idx : -1).filter(idx => idx !== -1);
  /* istanbul ignore next: n番目の要素が存在しない場合の防御的プログラミングだが、通常は適切なnが渡されるため、-1を返すケースは稀 */
  return oneIndices.length >= n ? oneIndices[n - 1] : -1;
}
 ...
 */
export function shiftArrayAsIndex<T>(array: T[], index: number) {
  const forward: T[] = array.slice(0, index);
  const backward: T[] = array.slice(index, array.length + 1);
 ...
 */
export function shiftArray<T>(array: T[], target: T) {
